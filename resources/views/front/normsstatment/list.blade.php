@extends('layouts.front')

@section('content')

    <!-- aboutUniversity start -->

    <div class="aboutUniversity">
        <section class="container">
            <div class="aboutUniversity__cart">
                <h2 class="about__title__h2">Norms & statements</h2>
                <ul class="aboutUniversity__menu">
                    <li>
                        <a href="index.html" class="aboutUniversity__menu__link">Main</a>
                    </li>

                    <li>
                        <a href="normsStatements.html" class="aboutUniversity__menu__link">Norms & statements</a>
                    </li>
                </ul>
            </div>
        </section>
    </div>

    <!-- aboutUniversity end -->


    <!-- normsStatements start -->

    <div class="normsStatements">
        <section class="container">
            <div class="normsStatements__cart">
                <div class="educational__list">

                    <div class="educational__item">
                        <a href="normsStatements_in.html">
                            <div class="educational__img">
                                <img src="foto/normsStatements.png" alt="educational">
                            </div>

                            <h3 class="educational__title__h3">
                                Banking and auditing
                            </h3>

                            <h4 class="educational__icons">
                                <svg width="21" height="38" viewBox="0 0 21 38" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M2 36L19.0711 19L2 2" stroke="#133654" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>
                                </svg>
                            </h4>
                        </a>
                    </div>

                    <div class="educational__item">
                        <a href="normsStatements_in.html">
                            <div class="educational__img">
                                <img src="foto/normsStatements.png" alt="educational">
                            </div>

                            <h3 class="educational__title__h3">
                                Banking and auditing
                            </h3>

                            <h4 class="educational__icons">
                                <svg width="21" height="38" viewBox="0 0 21 38" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M2 36L19.0711 19L2 2" stroke="#133654" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>
                                </svg>
                            </h4>
                        </a>
                    </div>

                    <div class="educational__item">
                        <a href="normsStatements_in.html">
                            <div class="educational__img">
                                <img src="foto/normsStatements.png" alt="educational">
                            </div>

                            <h3 class="educational__title__h3">
                                School management
                            </h3>

                            <h4 class="educational__icons">
                                <svg width="21" height="38" viewBox="0 0 21 38" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M2 36L19.0711 19L2 2" stroke="#133654" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>
                                </svg>
                            </h4>
                        </a>
                    </div>

                    <div class="educational__item">
                        <a href="normsStatements_in.html">
                            <div class="educational__img">
                                <img src="foto/normsStatements.png" alt="educational">
                            </div>

                            <h3 class="educational__title__h3">
                                Management (by industries and sectors)
                            </h3>

                            <h4 class="educational__icons">
                                <svg width="21" height="38" viewBox="0 0 21 38" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M2 36L19.0711 19L2 2" stroke="#133654" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>
                                </svg>
                            </h4>
                        </a>
                    </div>

                    <div class="educational__item">
                        <a href="normsStatements_in.html">
                            <div class="educational__img">
                                <img src="foto/normsStatements.png" alt="educational">
                            </div>

                            <h3 class="educational__title__h3">
                                Ut enim ad minim veniam
                            </h3>

                            <h4 class="educational__icons">
                                <svg width="21" height="38" viewBox="0 0 21 38" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M2 36L19.0711 19L2 2" stroke="#133654" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>
                                </svg>
                            </h4>
                        </a>
                    </div>

                    <div class="educational__item">
                        <a href="normsStatements_in.html">
                            <div class="educational__img">
                                <img src="foto/normsStatements.png" alt="educational">
                            </div>

                            <h3 class="educational__title__h3">
                                Quis nostrud exercitation
                            </h3>

                            <h4 class="educational__icons">
                                <svg width="21" height="38" viewBox="0 0 21 38" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M2 36L19.0711 19L2 2" stroke="#133654" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>
                                </svg>
                            </h4>
                        </a>
                    </div>

                </div>
            </div>
        </section>
    </div>

    <!-- normsStatements end -->


   @endsection
